...
function checkFile(source: string): vscode.Diagnostic[] {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function refreshDiagnostics(document: vscode.TextDocument) {
    if (document.languageId !== "pactia") return;
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider("pactia", {
        ...
        for (const diag of context.diagnostics) {
          if (diag.source?.includes("IMPORT_MISSING")) {
            ...
            const fix = new vscode.CodeAction(
              `Add import { ${fullSymbol} } from @package`,
            ...
            fix.edit.insert(
              ...
              `import { ${fullSymbol} } from @package;\n`,
            );
